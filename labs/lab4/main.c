	...
	while (button != 'q') {
		...
		switch (button) {
			case '1' : {
				...
				Storage_writeResult(filename, result);
			...
			case '2' : {
				...
				Provider * list = Storage_readText(filename);
				while (button != 'r') {
					Cui_showEditMenu();
					Cui_printList(list);
					button = conGetChar();
					switch (button) {
						...
						case 's': {
							...
							getCorrectInputData(filepath, "Input FILENAME: ", 33, 60, FG_YELLOW, correctFileName);
							Storage_writeResult(filepath, list);
							button = 'r';
							break;
						}
						default: {
